...
def extrair_visitantes_excel():
    ...
    # URL atualizada para buscar os Visitantes
    url = 'https://web.appmykids.com.br/gileadeweb-api/api/pessoavisitante/gquery'
    headers = {
        'accept': 'application/json, text/plain, */*',
    ...
    print("Iniciando extração de dados de Visitantes da API...")

    while True:
        print(f"Buscando registros a partir da posição {start}...")
        
        # O payload dos visitantes tem uma estrutura de gQuery levemente diferente (com subQuerys)
        payload = {
            "start": start,
            "pageSize": page_size,
            "gQuery": {
                "subQuerys": [
                    {"subQuerys": [], "joins": [], "selects": [], "logicalOperator": "SIMPLE"},
                    {"subQuerys": [], "joins": [], "selects": [], "logicalOperator": "SIMPLE"}
                ],
                "joins": [],
                "selects": [],
    ...
    print(f"\nTotal de Visitantes extraídos com sucesso: {len(all_data)}")
    ...
    if all_data:
        ...
        # Ajustando algumas colunas principais baseadas no print
        primeiras_colunas = [c for c in ['id', 'nomeFonetico', 'telefonePrincipal', 'status'] if c in cols]
        outras_colunas = [c for c in cols if c not in primeiras_colunas]
        df = df[primeiras_colunas + outras_colunas]
        
        script_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(script_dir, 'dados_visitantes.xlsx')
        
        df.to_excel(file_path, index=False)
        print(f"✅ Arquivo Excel gerado com sucesso em: {file_path}")

if __name__ == '__main__':
    extrair_visitantes_excel()
